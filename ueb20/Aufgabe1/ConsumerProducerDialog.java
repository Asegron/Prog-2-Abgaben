...
public class ConsumerProducerDialog 
{
   private static final String AUFRUF
                          = "Aufruf:  java ConsumerProducerDialog  { 'fifo' | 'prio' }" ;
   ...
   public ConsumerProducerDialog ( String wahl )
   {
     if ( wahl.equals( "fifo" ) )
   ...
   public void start()
   {
      ...
      {
         if ( ran.nextInt( 2 ) > 0 )               
            tasks.add( new Integer( p.produce() ));     // produce
         else
         {
            try
            {
   ...
   public static void main( String[] args ) throws InterruptedException
   {
     ...
     {
        ConsumerProducerDialog  loesung = new ConsumerProducerDialog ( args[0] );
        loesung.start();
     }
     System.exit( 0 );
   }
